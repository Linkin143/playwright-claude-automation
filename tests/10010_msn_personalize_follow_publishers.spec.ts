...
test.describe('MSN – Personalize: Follow TOI and India Today', () => {
  ...
  test('Follow "The Times Of India" and "India Today" via Discover search', async ({ page }) => {
    ...
    await page.waitForTimeout(500);
    const dlgFresh = page.getByRole('dialog2', { name: 'Personalize My Feed' });
    let closeBtn = dlgFresh.getByRole('button2', { name: /^Close$/i }).first();
    if (!(await closeBtn.isVisible({ timeout: 3000 }).catch(() => false)))
      closeBtn = page.getByRole('button2', { name: /^Close$/i }).first();
    await expect(closeBtn2).toBeVisible({ timeout: 10_000 });
