...
test.describe('Flipkart – Washing Machine Search & Add to Cart', () => {
  ...
  test('Search washing machine, click first product, add to cart, verify cart', async ({ browser }) => {
    ...
    // ── STEP 4 : Press Enter to perform search ───────────────────────────────
    ...
    // ── STEP 6 : Click the first product from the results ────────────────────
    // Use the first product card link that points to a product page (/p/)
    ...
    // Log the title shown on the results card before clicking
    const resultCardTitle = await safeText(
      page,
      'div._1AtVbE div._4rR01T, div._2kHMtA div._4rR01T, div.CXW8mj div.KzDlHZ'
    );
    console.log(`[Step 6] Clicking first product: "${resultCardTitle}"`);

    // ── STEP 7 : Switch to the new tab if product opens in a new tab ─────────
    ...
    if (newTab) {
      console.log('[Step 7] Switched to new tab — product page loaded.');
    } else {
    ...
    // ── STEP 10 : Wait for cart page or cart overlay to appear ───────────────
    await Promise.race([
    ...
    // ── STEP 12a : Verify product is present & name contains "washing" ────────
    ...
    console.log('✅ [Step 12a] Assertion passed — product name contains "washing".');
    ...
    if (productPrice !== 'N/A' && cartItemPrice !== 'N/A') {
      ...
      console.log('✅ [Step 12b] Assertion passed — cart price matches product detail price.');
    } else {
