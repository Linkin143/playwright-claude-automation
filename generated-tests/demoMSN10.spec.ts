...
// Self-healing: exact placeholder scoped → searchbox scoped → textbox scoped
//               → exact placeholder unscoped → searchbox unscoped
// NOTE: NO waitForLoadState — wait for specific UI element visibility instead
// ─────────────────────────────────────────────────────────────────────────────
async function resolveSearchBox(page: Page, container: Locator): Promise<Locator> {
  // PRIMARY: exact placeholder scoped to panel container
  ...
  if (!(await el.isVisible({ timeout: 4000 }).catch(() => false))) {
    // FALLBACK 3: exact placeholder unscoped (panel may not have ARIA role)
    el = page.getByPlaceholder('Search for channels to follow');
  }
  if (!(await el.isVisible({ timeout: 4000 }).catch(() => false))) {
    // FALLBACK 4: unscoped searchbox — last resort
...
// ─────────────────────────────────────────────────────────────────────────────
// HELPER : Wait for search results to appear (NO networkidle — UI element wait)
// Waits for a list/listitem or result container inside the panel to appear
// ─────────────────────────────────────────────────────────────────────────────
async function waitForSearchResults(page: Page, container: Locator): Promise<void> {
  // Wait for at least one list item (channel result) to become visible
  let resultItem: Locator = container.getByRole('listitem').first();
  const resultVisible = await resultItem.isVisible({ timeout: 8000 }).catch(() => false);
  if (resultVisible) {
    await expect(resultItem).toBeVisible({ timeout: 8000 });
    console.log('[Results] Search results appeared (listitem).');
    return;
  }
  // FALLBACK: wait for list container
  const resultList = container.getByRole('list').first();
  const listVisible = await resultList.isVisible({ timeout: 6000 }).catch(() => false);
  if (listVisible) {
    await expect(resultList).toBeVisible({ timeout: 6000 });
    console.log('[Results] Search results appeared (list).');
    return;
  }
  // FALLBACK: short timeout as safety net only — not networkidle
  await page.waitForTimeout(2500);
  console.log('[Results] Waited via timeout fallback.');
}

// ─────────────────────────────────────────────────────────────────────────────
// HELPER : Resolve follow/add "+" button SCOPED to container (always call FRESH)
// Self-healing: "+" → "Add" → "Follow" → label:Follow → label:Add
// .first() is correct — multiple follow buttons appear in results list
// ─────────────────────────────────────────────────────────────────────────────
...
test.describe('MSN India – Personalize: Follow Channels via Search', () => {
  ...
  test('Follow The Times Of India and India Today via Personalize search', async ({ page }) => {
    ...
    // Wait for domcontentloaded only — no networkidle for SPA
    ...
    // Wait for panel to open — detected by UI element, NOT networkidle
    ...
    // ── STEP 4 : Locate and use search box — "The Times Of India" ────────────
