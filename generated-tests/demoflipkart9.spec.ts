...
test.describe('Flipkart – Sponsored Washing Machine Search & Add to Cart', () => {
  ...
  test('Find first Sponsored product, open it, add to cart and verify', async ({ browser }) => {
    ...
    // ── STEP 3 : Type "washing machine" in the search bar ────────────────────
    ...
    // ── STEP 6 : Find first product card whose text contains "Sponsored" ──────
    // Strategy: collect every product card wrapper, iterate, check innerText
    // for the word "sponsored" (case-insensitive), then grab its first link.
    console.log('[Step 6] Scanning result cards for "Sponsored" label...');

    const allCards = page.locator('div._1AtVbE, div._2kHMtA, div.CXW8mj');
    const cardCount  = await allCards.count();
    console.log(`[Step 6] Total result cards found: ${cardCount}`);

    let sponsoredLink: ReturnType<Page['locator']> | null = null;
    let sponsoredIndex = -1;
    let sponsoredTitleOnResults = 'N/A';
    ...
    for (let i = 0; i < cardCount; i++) {
      ...
      if (cardText.includes('sponsored')) {
        ...
        const isVisible  = await linkInCard.isVisible({ timeout: 2000 }).catch(() => false);

        if (isVisible) {
          sponsoredLink  = linkInCard;
          sponsoredIndex = i;
          sponsoredTitleOnResults = (
            await card.locator('div._4rR01T, div.KzDlHZ, a[href*="/p/"]')
              .first().textContent().catch(() => 'N/A')
          ).trim();
          console.log(`[Step 6] Sponsored card found at index ${i}: "${sponsoredTitleOnResults}"`);
          break;
        }
      }
    }

    // Hard-fail early if no Sponsored card was found
    if (!sponsoredLink) {
      throw new Error(
        '[Step 6] FAIL – No product card containing the "Sponsored" label ' +
        'was found on the results page. The test cannot continue.'
      );
    }
    ...
    // ── STEP 12b : Verify cart price matches product detail page price ────────
    ...
    console.log(`║  Sponsored Card Index: ${sponsoredIndex}`);
    console.log(`║  Result Card Title   : ${sponsoredTitleOnResults}`);
